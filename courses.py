...
def create_tables():
    ...
    db.execute("CREATE TABLE Suoritukset (id INTEGER PRIMARY KEY, kurssi_id INTEGER REFERENCES Kurssit, op_id INTEGER REFERENCES Henkilot, pvm DATETIME, arvosana INTEGER)")
...
def add_credits(student_id, course_id, date, grade):
    db.execute("INSERT INTO Suoritukset (kurssi_id, op_id, pvm, arvosana) VALUES (?, ?, ?, ?)", [course_id, student_id, date, grade])
...
def courses_by_teacher(teacher_name):
    placeholder = db.execute("SELECT K.nimi FROM Kurssit K, Henkilot H, KurssinOpettajat KO WHERE KO.kurssi_id=K.id AND KO.opettaja_id=H.id AND H.nimi=?", [teacher_name]).fetchall()
    return [i[0] for i in placeholder]

# hakee opettajan antamien opintopisteiden määrän
def credits_by_teacher(teacher_name):
    credits = db.execute("SELECT SUM(K.op) FROM Kurssit K, Henkilot H, KurssinOpettajat KO, Suoritukset S WHERE S.kurssi_id=K.id AND KO.kurssi_id=K.id AND KO.opettaja_id=H.id AND H.nimi=?", [teacher_name]).fetchone()
    return credits[0]

# hakee opiskelijan suorittamat kurssit (aakkosjärjestyksessä)
def courses_by_student(student_name):
    return db.execute("SELECT K.nimi, S.arvosana FROM Kurssit K, Henkilot H, Suoritukset S WHERE K.id = S.kurssi_id AND S.op_id = H.id and H.nimi=? ORDER BY 1 ASC", [student_name]).fetchall()

# hakee tiettynä vuonna saatujen opintopisteiden määrän
def credits_by_year(year):
    year =str(year)
    credits = db.execute("SELECT SUM(K.op) FROM Kurssit K, Suoritukset S WHERE K.id = S.kurssi_id AND strftime('%Y', S.pvm)= ?", [year]).fetchone()
    return credits[0]
...
def grade_distribution(course_name):
    dict={}
    for i in range(1, 6):
        result=db.execute("SELECT COUNT(S.arvosana) FROM Suoritukset S, Kurssit K WHERE S.arvosana = ? AND S.kurssi_id = K.id AND K.nimi=?", [i, course_name]).fetchone()
        dict[i]=result[0]
    return dict
...
def credits_in_groups():
    return db.execute("SELECT R.nimi, IFNULL(SUM(K.op), 0) FROM Ryhmat R LEFT JOIN RyhmanJasenet RJ ON R.id = RJ.ryhma_id LEFT JOIN Henkilot H ON RJ.henkilo_id = H.id LEFT JOIN Suoritukset S ON H.id =S.op_id LEFT JOIN Kurssit K ON S.kurssi_id = K.id GROUP BY R.nimi ORDER BY 1 ASC").fetchall()

# hakee ryhmät, joissa on tietty opettaja ja opiskelija (aakkosjärjestyksessä)
def common_groups(teacher_name, student_name):
    groups = db.execute("SELECT R.nimi FROM Ryhmat R, RyhmanJasenet RJ, RyhmanJasenet RK WHERE R.id = RJ.ryhma_id AND R.id = RK.ryhma_id AND RJ.ryhma_id IN (SELECT RJ.ryhma_id FROM RyhmanJasenet RJ, Henkilot H WHERE H.nimi=? AND H.id = RJ.henkilo_id) AND RK.ryhma_id IN (SELECT RK.ryhma_id FROM RyhmanJasenet RK, Henkilot H WHERE H.nimi=? AND H.id = RK.henkilo_id) GROUP BY R.nimi", [teacher_name, student_name]).fetchall()
